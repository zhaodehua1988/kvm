...
*******************************************************************/
 WV_S32 SYS_IP_getIpInt(WV_U8* pIp)
{

	WV_S8 pSrc[20];
	SYS_ENV_Get("IP_Ip",pSrc);
	WV_S8* pData = pSrc;	
	WV_S32 len;
	WV_S32 i,j,k,data=0;
	WV_S32 des;
...
{ 
   ...
   SYS_IP_GetConf();  
   ret = SYS_IP_CheckIP();
    WV_CMD_Register("ip",NULL,"ip get or set ",NULL); 
    WV_CMD_Register("ip","get","print ip info",SYS_IP_CMDGet);  
    WV_CMD_Register("ip","set","set ip config",SYS_IP_CMDSet); 
   if(ret != WV_SOK)
   {
     SYS_IP_SaveConf();
     return WV_EFAIL; 

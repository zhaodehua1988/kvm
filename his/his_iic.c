...
WV_S32 HIS_REG_CmdGet(WV_S32 argc, WV_S8 **argv,WV_S8 *prfBuff)
...
WV_S32 HIS_REG_CmdGet(WV_S32 argc, WV_S8 **argv, WV_S8 * prfBuff)
{
	...
	if(argc< 1) 
		{
		prfBuff += sprintf(prfBuff,"get reg <addr> \r\n");
		return 0;
		}
	...
	prfBuff += sprintf(prfBuff,"adr=%08x\r\n",regAddr); 
	
	ret = HI_MPI_SYS_GetReg(regAddr, &regData);
	if(ret != WV_SOK)
	{
		prfBuff += sprintf(prfBuff,"adr erro!\r\n"); 
	}
	prfBuff += sprintf(prfBuff,"get reg[%8x]=[%8x]!\r\n",regAddr,regData); 
	return WV_SOK;
...
WV_S32 HIS_VPLL_CmdGet(WV_S32 argc, WV_S8 **argv,WV_S8 *prfBuff)

****************************************************************************/
WV_S32 HIS_VPLL_CmdGet(WV_S32 argc, WV_S8 **argv, WV_S8 * prfBuff)
{

	WV_U32   regAddr=0x12040010;
	WV_U32   regData1=0;
	WV_U32   regData2=0;

	WV_U32   num;
	WV_S32 ret;
	WV_U32 fbdiv,frac,pstdiv1,pstdiv2,fout,refdiv;
	if(argc< 1) 
	{
		prfBuff += sprintf(prfBuff,"get vpll <num> \r\n");
		return 0;
	}
	ret =  WV_STR_S2v(argv[0],&num);
	if(ret != WV_SOK)
	{
		prfBuff += sprintf(prfBuff,"input erro!\r\n"); 
	}
	
	ret = HI_MPI_SYS_GetReg(regAddr+num*8, &regData1);
	ret = HI_MPI_SYS_GetReg(regAddr+num*8+4, &regData2);
	...
	pstdiv1 = (regData1>>24) & 0x7;
	pstdiv2 = (regData1>>28) & 0x7;
	fbdiv = regData2 & 0xfff;
	refdiv = (regData2 >> 12) &0x3f;

	fout = 12*(fbdiv)/pstdiv1/pstdiv2/refdiv;

	prfBuff += sprintf(prfBuff,"get pll=[%d]MHz,reg0[%08x],reg1[%08x]!\r\n",fout,regData1,regData2); 
	return WV_SOK;
 	
}
/****************************************************************************

WV_S32 HIS_REG_CmdSet(WV_S32 argc, WV_S8 **argv, WV_S8 * prfBuff)
...
WV_S32 HIS_REG_CmdSet(WV_S32 argc, WV_S8 **argv, WV_S8 * prfBuff)
{
	...
	if(argc< 2) 
	{
		prfBuff += sprintf(prfBuff,"set reg <addr> <data>\r\n");
		return 0;
	}
	ret =  WV_STR_S2v(argv[0],&regAddr);
	if(ret != WV_SOK)
	{
		prfBuff += sprintf(prfBuff,"input addr error!\r\n"); 
	}
	
	ret =  WV_STR_S2v(argv[1],&regData);
	if(ret != WV_SOK)
	{
		prfBuff += sprintf(prfBuff,"input regData error!\r\n"); 
	}	

	ret = HI_MPI_SYS_SetReg(regAddr, regData);
	if(ret != WV_SOK)
	{
		prfBuff += sprintf(prfBuff,"set reg  error!\r\n"); 
	}
	
		prfBuff += sprintf(prfBuff,"set reg  addr[%08x]=[%08x]\r\n",regAddr,regData); 	
	return 0;
}
...
{	  
    ...
    WV_CMD_Register("get","reg","get reg  read",HIS_REG_CmdGet);
    WV_CMD_Register("set","reg","set reg  read",HIS_REG_CmdSet);
    WV_CMD_Register("get","pll","get pll  read",HIS_VPLL_CmdGet);

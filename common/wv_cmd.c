...
WV_S32 	 WV_CMD_GetVer(WV_S32 argc, WV_S8 ** args,WV_S8 *prfBuff)
{
  ...
  prfBuff += sprintf(prfBuff,"%s%s[%s]:%s(%s-%s)[%s]\r\n",SOFTVER_VENDOR,SOFTVER_TYPE,SOFTVER_NAME,SOFTVER_MAIN,SOFTVER_DATE,SOFTVER_TIME,SOFTVER_NUM); 
    return  WV_SOK;  
}
...
WV_S32 	 WV_CMD_Create()
{
	...
	WV_CMD_Register("sys","ver","get sys version",WV_CMD_GetVer); 
	WV_CMD_Register("sys","bye","bye telnet",WV_CMD_SysBye); 
	WV_CMD_Register("sys","reboot","sys reboot",WV_CMD_SysReboot);
	...
	WV_CMD_Register("q",NULL,"exit app",WV_CMD_SysExit);  
return WV_SOK;
}
